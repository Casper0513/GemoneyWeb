...
export const CONTEXT_Provider = ({ children }) => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      //GEMONEYCOINTOKEN_TOKEN_CONTRACT
      const GEMONEYCOINTOKEN_TOKEN_CONTRACT = await internalGemoneycoinTokenContract();

      let tokenBalance;
      if (account) {
        tokenBalance = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.balanceOf(account);
      } else {
        tokenBalance = 0;
      }

      const tokenName = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.name();
      const tokenSymbol = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.symbol();
      const tokenTotalSupply = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.totalSupply();
      const tokenStandard = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.standard();
      const tokenHolders = await GEMONEYCOINTOKEN_TOKEN_CONTRACT._userId();
      const tokenOwnerOfContract = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.ownerOfContract();
      const tokenAddress = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.address;
      ...
      const getTokenHolder = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.getTokenHolder();
      setTokenHolders(getTokenHolder);

      //GETTING TOKEN HOLDER DATA
      if (account) {
        const getTokenHolderData = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.getTokenHolderData(
          account
        );
      ...
      //TOKEN SALE CONTRACT
      const ICO_GEMONEYCOINTOKEN_CONTRACT = await internalICOGemoneycoinTokenContract();
      const tokenPrice = await ICO_GEMONEYCOINTOKEN_CONTRACT.tokenPrice();
      const tokenSold = await ICO_GEMONEYCOINTOKEN_CONTRACT.tokensSold();
      const tokenSaleBalance = await GEMONEYCOINTOKEN_TOKEN_CONTRACT.balanceOf(
        "0x3A61E507e4d38036aE58aF1087e6c123eC956669"
      );
  ...
  const buyToken = async (nToken) => {
    try {
      ...
      const contract = await internalICOGemoneycoinTokenContract();
  ...
  const transferNativeToken = async () => {
    try {
      ...
      const contract = await internalGemoneycoinTokenContract();
